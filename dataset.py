...
class StoicDataset(Dataset):
    ...
    def __init__(self,
        ...
        """
        super(StoicDataset, self).__init__()
        ...
        self.mha_path = os.path.join(root_directory, dir_name, 'mha')
        self.mask_path = os.path.join(root_directory, dir_name, 'lungmasks')

        self.clinical_data = pd.read_csv(clinical_data_path)
        if split == 'train':
            self.clinical_data = self.clinical_data[self.clinical_data['split'] == 'train'][self.clinical_data['fold'] != train_fold]
        elif split == 'val':
            self.clinical_data = self.clinical_data[self.clinical_data['split'] == 'train'][self.clinical_data['fold'] == train_fold]
            self.dataaug = False
        else: #split == 'test
            self.clinical_data = self.clinical_data[self.clinical_data['split'] == 'test']
            self.dataaug = False
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, float]:
        ...
        mrn     = str(int(row.PatientID))
        covid   = torch.tensor(row.probCOVID, dtype=torch.float32)
        severe  = torch.tensor(row.probSevere, dtype=torch.float32)
        
        label   = torch.tensor([covid, severe], dtype=torch.float32)
        img     = sitk.ReadImage(os.path.join(self.mha_path, f"{mrn}.mha"))
        mask    = sitk.ReadImage(os.path.join(self.mask_path, f"{mrn}.mha"))

        mask    = binarize_mask(mask)
        age     = img.GetMetaData('PatientAge')
        ...
        if self.dataaug:
            transform = tio.Compose([tio.RandomFlip(axes=('LR', 'AP')), 
                                    tio.RandomAffine(degrees=(15, 15, 0)),
